...
#### 5.4.3 Baseline Scenario — Family-Aligned Targets (Oct 23 2025)

- **Config:** `experiments/configs/explorer/baseline_trace_gsm8k.yaml` is the canonical sweep entry point. It runs the closed-loop simulator in **blocking** mode while the speculation block stays **vanilla distributed**, which keeps the trace replay reproducible but still exercises the distributed policy plumbing.
- **Acceptance model:** The drafter/voter pair now loads the `acceptance/Llama-2-7b-hf_vs_Llama-2-70b-hf-gbdt.joblib` checkpoint produced by the gradient-boosting pipeline (classifier-only fast path; regressor disabled). Update this path whenever a new acceptance bundle becomes the default.
- **Targets & parallelism:** Each cluster tier (LLaMA‑2 70B, CodeLlama‑34B, Qwen‑72B, LLaMA‑3 70B) advertises VIDUR profiles with `tensor_parallel: 2` and `pipeline_parallel: 2`, effectively modeling 4 GPU shards per verifier pool so prefill/decode stages saturate multi-GPU capacity during sweeps.
- **Draft connectivity:** A40 drafter buckets retain their capability splits but now draw RTTs from label-specific ranges spanning 10 ms → 180 ms (fast buckets: 10–30 ms, slow 1B buckets: 150–180 ms). Auto-topology fans each drafter out to four targets and stamps the sampled RTT onto every draft↔target edge, so latency plots reflect heterogeneous client links without bespoke trace edits.
- **Motivation:** Keeping tokenizer families isolated (LLaMA-only cluster today) ensures VIDUR assets stay valid while still stressing acceptance policies under realistic RTT skew and multi-GPU verifier pools. This profile is the baseline for `gamma_policy_draft_sweep.py`, RTT sweeps, and routing experiments until we land a refreshed multi-cluster layout.
...
- **Integration.** Expose `speculation.gamma_policy.type: latency_head` (alias `awc`) to wire the history buffers, feature extraction, and predictor invocation. Config knobs define history length, coefficient vector / MLP weights, smoothing window, and fallback defaults when history is empty.

##### Telemetry & Feature Vector

- **Per-request snapshot.** At every decode decision point we materialise all signals the Adaptive Window Controller uses: instantaneous queue utilisation, per-target backlog (pending tokens + batch depth), drafter tier/capability, the EMA of recent acceptance rate, and the previously served gamma. Alongside the “current” snapshot, we retain the trailing history window (default 3 completions) for per-connection RTT and TPOT so the predictor can reason about trends (`avg`, `delta` for both series). These 11 scalars form the canonical feature vector `{queue_util, queue_trend, pending_norm, rtt_avg_norm, rtt_delta, tpot_avg_norm, tpot_delta, drafter_capability_norm, acceptance_recent, last_gamma_norm, context_norm}`.
- **History buffers.** Each `(draft, target)` pair keeps a deque (length = 3) with `(avg_rtt_ms, avg_tpot_ms, gamma_used)` while each target maintains its own queue-util deque. Buffers are lazily created and flushed on cold start; until two samples are present the policy falls back to the default distributed gamma to avoid extrapolating from empty history.
...
- **Fixed topology per scenario.** GSM8K/CNNDM/HumanEval runs still use the 400-draft heterogeneous cluster (ten A40 tiers feeding 20 verify targets). HumanEval keeps its 164 prompts; GSM8K and CNNDM now use 200-prompt traces to shorten turnaround.
- **Condition sweeps.** We raised `network_config.base_rtt_ms` to 10 ms and sweep RTT multipliers `{2, 3, 5, 10, 15}`, yielding effective RTTs from 20 ms up to 150 ms so the dataset covers slow networks where fused mode dominates. Offered load remains fixed (per-draft RPS=1) for this tranche; a future expansion can add load multipliers.
- **Oracle logging.** Every gamma candidate run enables `speculation.gamma_oracle_logging`, so each conversation logs the live feature vector, per-request metrics (TTFT/TPOT/latency samples), `run_metadata` (`scenario_id`, `drafter_count`, `rtt_multiplier`, etc.), and the actual `gamma` used during that constant-gamma simulation.
- **Per-arrival oracle.** Offline aggregation replicates the “oracle” selection by joining all candidate recordings per `request_id` and choosing the lowest-duration action that passes TTFT (`ttft_slo`) and TPOT (`tpot_slo`) thresholds. The result is a JSON row `{features, best_gamma, best_mode, metrics, run_metadata, candidates}` written to `data/gamma_oracle/dataset/<scenario>.jsonl`.
- **Dataset targets.** Current default sweep (200/200/100 prompts, 4 drafter counts × 5 RTT multipliers × 8 gammas) produces 128 raw JSONLs per scenario and ~600 labelled rows overall—sufficient for the initial head. All raw logs are kept under `data/gamma_oracle/raw/<scenario>/scenario_<label>_gamma<k>.jsonl`, while historical runs are backed up before re-sweeps so we can merge more regimes later.

##### Training Pipeline

- **Trainer.** `src/experiments/train_dynamic_gamma_predictor.py` consumes the raw JSONLs (rather than the aggregated dataset) to ensure we reconstruct history exactly like the runtime. It replays each scenario in order, maintains the same deques/EMAs as the policy, extracts the 11-feature vector, and uses the oracle-selected `best_gamma` as the target. Training uses PyTorch/Adam, outputs both a `.json` bundle (with `proj`, `block1`, `block2`, and `output` weights/biases) and an optional `.pt` checkpoint, and reports MAE/RMSE.
- **Consumption.** Point `speculation.gamma_policy.mlp.state_path` at the exported JSON and the simulator will automatically load the weights at startup; no manual editing of matrices is required.
